...
void createCliffordTable(const BASE_BLADE& _bladesBase, const Orthonormal& ortonormal, OPERATION _type, CLIFFORD_TABLE& _table){
   ...
   for(BASE_BLADE::size_type i = 0; i < _bladesBase.size(); i++)
      for(BASE_BLADE::size_type j = 0; j < _bladesBase.size(); j++){
          switch (_type) {
          case OPERATION::GP: _table[i][j] = GP(_bladesBase[i],bladesBase[j],ortonormal); break;
          case OPERATION::SCP: _table[i][j] = SCP(_bladesBase[i],bladesBase[j],ortonormal); break;
          case OPERATION::LCONST: _table[i][j] = LConst(_bladesBase[i],bladesBase[j],ortonormal); break;
          case OPERATION::RCONST: _table[i][j] = RConst(_bladesBase[i],bladesBase[j],ortonormal); break;
          default:
              break;
          }
      }
